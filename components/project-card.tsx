...
export function ProjectCard({ title, description, tags = [], image, demoUrl, repoUrl, slug }: ProjectCardProps) {
  ...
  const cardContent = (
    <div
      className="relative h-full overflow-hidden rounded-xl bg-zinc-800/50 backdrop-blur-sm border border-zinc-700/50 transition-all duration-300 group-hover:border-purple-500/50 cursor-pointer"
    ...
    >
      ...
      <div className="relative h-full flex flex-col">
        ...
        <div className="p-6 flex-grow">
          <h3 className="text-xl font-bold mb-2 hover:text-purple-400 transition-colors">{title}</h3>
  ...
  return (
    ...
    >
      <div
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" && slug) router.push(`/projects/${slug}`)
        }}
      >
        {cardContent}
      </div>
