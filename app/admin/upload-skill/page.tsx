...
export default function UploadSkillPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.from("skills").insert([formData])
        ...
        <div className="min-h-screen bg-gradient-to-b from-zinc-900 via-zinc-900 to-black text-white flex items-center justify-center">
          <Card className="bg-zinc-800 border-zinc-700 max-w-md">
            <CardHeader>
              ...
              <CardDescription>Please set up your Supabase environment variables to add skills.</CardDescription>
            </CardHeader>
          </Card>
        </div>
      </AuthGuard>
    ...
    <AuthGuard>
      <div className="min-h-screen bg-gradient-to-b from-zinc-900 via-zinc-900 to-black text-white">
        <div className="container py-8">
          <AdminHeader title="Add New Skill" description="Add a new skill to your expertise" />

          <div className="max-w-2xl mx-auto">
            <Card className="bg-zinc-800/50 border-zinc-700 text-zinc-100">
              <CardHeader>
                <CardTitle>Skill Details</CardTitle>
                <CardDescription>Add a new skill with its name, website URL, and optional icon</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    {formData.icon_url && (
                      <div className="flex items-center gap-4 p-4 rounded-lg bg-zinc-900/50 border border-zinc-700">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium">Icon uploaded successfully</p>
                          <p className="text-xs text-zinc-400">Click upload again to replace</p>
                        </div>
                        <Button
                          type="button"
                          variant="outline"
